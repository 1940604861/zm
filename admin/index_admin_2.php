...
$sql="select * from guang limit {$offset},{$num}";
$result=mysql_query($sql);
...
echo '<table align="center" width="1090px" border="2px" bordercolor="#6699CC" >';
    echo'<tr>';
		 echo'<td colspan="3">光源电器</td>';
		 echo'<td ><a href="index_admin_2_add.php?id=<?php echo $row[0];?>" target="_blank">添加</a></td>';
         echo'</tr>';
...
?></td>
	 <td><a href="index_admin_2_edit.php?id=<?php echo $row[0];?>" target="_blank">修改</a>| 
	      <a href="index_admin_2_del.php?id=<?php echo $row[0];?>" target="_blank">删除</a>|
     </td>
<?php echo '</tr>';$x++;}
...
echo "共<b>$total</b>条记录，本页显示<b>{$start}-{$end}</b> {$cpage}/{$pagenum}";
	if($cpage==1)
		...
		echo "  <a href='index_admin.php?y=2&page=1'>首页</a>  ";
	if($prev)
		echo "  <a href='index_admin.php?y=2&page={$prev}'>上一页</a>  ";
	else
		echo "  上一页  ";
	if($next)
		echo "  <a href='index_admin.php?y=2&page={$next}'>下一页</a>  ";
	else
		echo "  下一页  ";
	if($cpage==$pagenum)
		echo "  尾页  ";
	else
		echo "  <a href='index_admin.php?y=2&page={$pagenum}'>尾页</a>  ";
echo '</td></tr>';
echo '</table>';
mysql_close();
?>
